...
export class SinistreFormComponent implements OnInit {
  ...
  // 🆕 GETTER : Le contrat est-il AUTO ?
  get isContratAuto(): boolean {
    return this.contratSelectionne?.typeContrat === 'AUTO';
  }
  ...
  onContratChange(): void {
    ...
    if (!contrat) {
      this.contratSelectionne = null;
      this.fichiers = [];  // 🆕 Reset si on retire le contrat
      return;
    }

    // 🆕 Mémoriser le contrat
    this.contratSelectionne = contrat;

    // 🆕 Reset des fichiers si on change pour un type NON-AUTO
    if (contrat.typeContrat !== 'AUTO') {
      this.fichiers = [];
    }
  ...
  onSubmit(): void {
    ...
    this.sinistreService.declarer(this.dto, this.fichiers).subscribe({
      next: () => {
        ...
        this.loading = false;
        this.cdr.detectChanges();
        const route = this.isClient ? '/client/sinistres' : '/admin/sinistres'; // ← ajouté
